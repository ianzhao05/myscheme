    ...
    match kw {
        ...
        "let" | "let*" | "letrec" => {
            ...
            match first {
                Datum::Compound(CompoundDatum::List(ListKind::Proper(_))) | Datum::EmptyList => {
                    let bindings = if let Datum::Compound(CompoundDatum::List(ListKind::Proper(
                        binding_data,
                    ))) = first
                    {
                        binding_data
        ...
        "do" => {
            ...
            match first {
                Datum::Compound(CompoundDatum::List(ListKind::Proper(_))) | Datum::EmptyList => {
                    let specs =
                        if let Datum::Compound(CompoundDatum::List(ListKind::Proper(spec_data))) =
                            first
                        {
                            spec_data
                                .into_iter()
...
mod tests {
    ...
    fn ands_ors() {
        ...
        assert_eq!(
            ...
            Ok(ExprOrDef::Expr(Expr::Literal(LiteralKind::SelfEvaluating(
